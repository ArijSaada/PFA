...
mysqli_set_charset($cnx, "utf8");
$sql1 = " SELECT c.* 
FROM cycle c, participant p 
WHERE c.theme = p.theme_part 
  AND p.mail = '$mail';
";
...
if ($requete->num_rows === 0) {
    echo json_encode(["status" => "success", "message" => "Account empty"]);
    exit();
} else {
    $response = [
        "status" => "success",
      ...
      while ($cycleData = mysqli_fetch_array($requete)) 
      {
        $response["data"][] = [
            "theme" =>$cycleData['theme'],
            "date_deb" =>$cycleData['date_deb'],
            "date_fin" =>$cycleData['date_fin'],
            "form" => $cycleData['form'],
            "num_salle" => $cycleData['num_salle']
